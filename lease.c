...
static long mx_lease_transition(struct mx_file_ctx *ctx, unsigned long arg)
{
	...
	old_state = *sm;
	old_holder = ctx->lease;
	if (ctx->lease.profile == MX_LEASE_PROFILE_COORDINATOR_INITIALIZER &&
	    req.operation == MX_LEASE_TRANSITION_ACTIVATE_COORDINATOR &&
				   ...
				   unsigned long arg)
{
	...
	if ((ctx->lease.profile == MX_LEASE_PROFILE_COORDINATOR_CONTROL ||
	     ctx->lease.profile == MX_LEASE_PROFILE_COORDINATOR_WORKLOAD) &&
	    (ctx->bar_mapping_count || ctx->direct_count)) {
		ret = -EBUSY;
		goto out_unlock;
	}
