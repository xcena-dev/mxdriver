...
static void mx_pdev_free(struct kref *ref)
{
	...
	/* Live BAR VMAs hold their own bar_map references and a device
	 * reference, so this runs only after the last of them closed. */
	if (mx_pdev->bar_map)
...
bool mx_pdev_get_live(struct mx_pci_dev *mx_pdev)
{
	...
	if (!mx_pdev || !kref_get_unless_zero(&mx_pdev->ref))
		return false;
	mutex_lock(&mx_pdev->lease.lock);
	live = !mx_pdev->lease.removed;
	mutex_unlock(&mx_pdev->lease.lock);
	if (!live)
		kref_put(&mx_pdev->ref, mx_pdev_free);
	return live;
}
...
int mx_lease_transfer_get(struct mx_file_ctx *ctx, struct mx_transfer *transfer)
{
	 ...
	 */
	ret = mx_lease_sm_validate_proofs(ctx->lease.profile,
			ctx->proofs_ever_bound, !!ctx->slot_proof_file,
...
static u64 mx_lease_holder_grants(const struct mx_file_ctx *ctx, bool anchored)
{
	...
	else if (family == MX_LEASE_FAMILY_COORDINATOR)
		grants |= MX_LEASE_GRANT_FAMILY_COORDINATOR;
	if (ctx->lease.profile == MX_LEASE_PROFILE_COORDINATOR_CONTROL)
		grants |= MX_LEASE_GRANT_COORDINATOR_CONTROL;
	...
	else if (ctx->lease.phase == MX_LEASE_PHASE_COORDINATOR_PREPARED)
		grants |= MX_LEASE_GRANT_COORDINATOR_PREPARED;
	if (anchored)
...
static long mx_lease_acquire(struct mx_file_ctx *ctx, unsigned long arg)
{
	...
	struct mx_device_lease *lease = &ctx->mx_pdev->lease;
	struct mx_lease_sm_holder old_holder, new_holder;
	...
	if ((mx_lease_profile_is_publisher(req.profile) ||
	     req.profile == MX_LEASE_PROFILE_COORDINATOR_CONTROL) &&
	...
	new_holder = ctx->lease;
	new_generation = lease->generation;
	mutex_unlock(&lease->lock);

	/* See mx_lease_transition(): publish outside the lock, undo only what is
	 * provably still ours. The holder is per-ctx and the fresh anchor is
	 * identified by the generation this call assigned. */
	...
	mutex_lock(&lease->lock);
	if (!memcmp(&ctx->lease, &new_holder, sizeof(new_holder)))
		mx_lease_sm_release(&lease->state, &ctx->lease);
	if (fresh_anchor && lease->state_path_valid &&
	    lease->generation == new_generation &&
	    lease->state_inode == file_inode(state_file) &&
	    mx_lease_sm_idle(&lease->state)) {
...
static long mx_lease_transition(struct mx_file_ctx *ctx, unsigned long arg)
{
	...
	struct mx_device_lease *lease = &ctx->mx_pdev->lease;
	struct mx_lease_sm *sm = &lease->state;
	struct mx_lease_sm old_state, new_state;
	struct mx_lease_sm_holder old_holder, new_holder;
	long ret;
	...
	/* Publish with the lock released: a fault here takes mmap_lock, which
	 * mmap/vm_close hold while waiting for this lock. Undo only if the state
	 * is still exactly what this call installed; anything else was changed
	 * by someone who saw the new state and now owns it. */
	if (!copy_to_user((void __user *)arg, &req, sizeof(req)))
		return 0;
	mutex_lock(&lease->lock);
	if (!memcmp(sm, &new_state, sizeof(*sm)) &&
	    !memcmp(&ctx->lease, &new_holder, sizeof(new_holder))) {
		*sm = old_state;
...
static bool mx_lease_valid_slot_proof(struct file *file, u32 slot, bool allow_zero)
{
	struct inode *inode;

	if (!file || (!allow_zero && slot == 0) || slot > MX_LEASE_MAX_SLOT ||
...
/* Which lock type a test probe reports, across the 6.9 file_lock split. */
static unsigned char mx_lease_probe_type(const struct file_lock *lock)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 9, 0)
	return lock->c.flc_type;
#else
	return lock->fl_type;
#endif
}

/* The canonical slot ABI keeps the identity byte (slot * 2) under an
 * exclusive OFD lock on the claimant's own open file description for the
 * claim's whole lifetime; makeLive() downgrades only the liveness byte. The
 * driver used to trust the marker in f_pos and never checked that, so two
 * descriptions that both skipped the exclusive claim were both admitted to one
 * slot -- and the bakery gate skips its own slot, so neither would ever see
 * the other. Two probes prove it without pinning anything on that byte:
 *   1. from the caller's own description a write probe must find no conflict
 *      (OFD locks never conflict with their own owner), and
 *   2. from a fresh kernel-only description the same probe must conflict.
 * Together they say the exclusive lock exists and belongs to this caller. */
static int mx_lease_slot_identity_held(struct file *slot_file, u32 slot)
{
	const loff_t offset = MX_LEASE_SLOT_IDENTITY_OFFSET(slot);
	struct file *clone;
	struct file_lock probe;
	int ret;

	mx_lease_init_ofd_lock(&probe, slot_file, F_WRLCK, offset, offset);
	ret = vfs_test_lock(slot_file, &probe);
	if (!ret && mx_lease_probe_type(&probe) != F_UNLCK)
		ret = -EPERM;	/* a stranger holds the identity byte */
	locks_release_private(&probe);
	if (ret)
		return ret;

	clone = file_clone_open(slot_file);
	if (IS_ERR(clone))
		return PTR_ERR(clone);
	if (file_inode(clone) != file_inode(slot_file)) {
		fput(clone);
		return -ESTALE;
	}
	mx_lease_init_ofd_lock(&probe, clone, F_WRLCK, offset, offset);
	ret = vfs_test_lock(clone, &probe);
	if (!ret && mx_lease_probe_type(&probe) == F_UNLCK)
		ret = -EPERM;	/* nobody holds it: the claim was never made */
	locks_release_private(&probe);
	fput(clone);
	return ret;
}
		...
		unsigned long arg)
{
	...
	/* Publish outside the lock (see mx_lease_transition). A fresh anchor is
	 * undone only while it is still the inode this call installed. */
	if (!copy_to_user((void __user *)arg, &req, sizeof(req)))
		goto out_files;
	mutex_lock(&lease->lock);
	if (fresh_anchor && lease->slot_domain_path_valid &&
	    lease->slot_domain_inode == file_inode(domain_file)) {
...
static bool mx_lease_valid_workload_ofd(struct file *file, u32 slot)
{
	struct inode *inode;

	if (!file || slot > MX_LEASE_MAX_WORKLOAD_SLOT ||
	    (file->f_mode & (FMODE_READ | FMODE_WRITE)) !=
		(FMODE_READ | FMODE_WRITE) ||
	    READ_ONCE(file->f_pos) != (loff_t)MX_LEASE_WORKLOAD_MARKER_BASE + slot)
		return false;
	inode = file_inode(file);
	return inode && S_ISREG(inode->i_mode) && inode->i_sb &&
	       inode->i_sb->s_magic == TMPFS_MAGIC;
				 ...
				 unsigned long arg)
{
	...
	struct file *lifetime_lock_file = NULL;
	struct inode *slot_inode = NULL;
	struct inode *lifetime_inode = NULL;
	bool same_binding;
	bool committed = false;
	const bool lifetime_ofd = req->reserved[0] != 0;
	const u32 lifetime_slot = req->reserved[0] & MX_LEASE_MAX_WORKLOAD_SLOT;
	long ret = 0;

	if (ctx->lease.profile == MX_LEASE_PROFILE_HUB_WORKLOAD ||
	    ctx->lease.profile == MX_LEASE_PROFILE_COORDINATOR_CONTROL) {
		bool control = ctx->lease.profile == MX_LEASE_PROFILE_COORDINATOR_CONTROL;

		if (control && req->lifetime_fd != -1) {
			ret = -EINVAL;
			goto out_files;
		}
		slot_file = fget(req->slot_fd);
		if (!mx_lease_valid_slot_proof(slot_file, req->slot, control)) {
			ret = -EINVAL;
			goto out_files;
		}
		if (req->lifetime_fd >= 0) {
		   ...
		   ctx->lease.profile == MX_LEASE_PROFILE_COORDINATOR_WORKLOAD) {
		if (req->slot_fd != -1 || req->slot != 0) {
			ret = -EINVAL;
			goto out_files;
		}
		lifetime_file = fget(req->lifetime_fd);
		if (lifetime_ofd ? !mx_lease_valid_workload_ofd(lifetime_file, lifetime_slot) :
				   !mx_lease_valid_lifetime_proof(lifetime_file)) {
			ret = -EINVAL;
			goto out_files;
		}
	} else {
	...
	same_binding = ctx->slot_source_file == slot_file &&
		       (lifetime_ofd ? ctx->lifetime_source_file : ctx->lifetime_proof_file) == lifetime_file &&
		       ctx->proof_slot == req->slot &&
		       ctx->proof_lifetime_tag == req->reserved[0];
	...
	if (!same_binding && slot_file) {
		ret = mx_lease_slot_identity_held(slot_file, req->slot);
		if (ret)
			goto out_unlock;
	...
	if (lifetime_ofd) {
		if (file_inode(lifetime_file) != lease->state_inode) {
			ret = -EXDEV;
			goto out_unlock;
		}
		if (!same_binding) {
			const loff_t offset = MX_LEASE_WORKLOAD_LIVENESS_OFFSET(lifetime_slot);

			lifetime_lock_file = mx_lease_clone_with_ofd_lock(lifetime_file,
						F_RDLCK, offset, offset);
			if (IS_ERR(lifetime_lock_file)) {
				ret = PTR_ERR(lifetime_lock_file);
				lifetime_lock_file = NULL;
				goto out_unlock;
			}
		}
	}
	...
	if (!same_binding) {
		...
		ctx->lifetime_source_file = lifetime_ofd ? lifetime_file : NULL;
		ctx->lifetime_proof_file = lifetime_ofd ? lifetime_lock_file : lifetime_file;
		ctx->proof_lifetime_tag = req->reserved[0];
		ctx->proof_slot = req->slot;
	...
	/* Publish outside the lock (see mx_lease_transition). If the copy faults,
	 * detach again only while the ctx still carries exactly the files this
	 * call installed; the locals then fput them below as on any failure. */
	if (!copy_to_user((void __user *)arg, req, sizeof(*req))) {
		if (committed) {
			slot_file = NULL;
			slot_lock_file = NULL;
	...
	if (committed) {
		mutex_lock(&lease->lock);
		if (ctx->slot_source_file == slot_file &&
		    ctx->slot_proof_file == slot_lock_file &&
		    ctx->lifetime_source_file == (lifetime_ofd ? lifetime_file : NULL) &&
		    ctx->lifetime_proof_file == (lifetime_ofd ? lifetime_lock_file : lifetime_file)) {
			ctx->slot_source_file = NULL;
			...
			/* Someone rebound or unbound meanwhile; those files are
...
out_files:
	...
	if (lifetime_file)
		fput(lifetime_file);
	if (lifetime_lock_file)
		fput(lifetime_lock_file);
	return ret;
}
				   ...
				   unsigned long arg)
{
	...
	struct file *lifetime_file = NULL;
	struct file *lifetime_source = NULL;
	u64 old_tag = 0;
	u32 old_slot = 0;
	long ret = 0;
	...
	if (req->reserved[0] != ctx->proof_lifetime_tag) {
		ret = -ESTALE;
		goto out_unlock;
	}
	/* Revoking the proof must not make an existing BAR mapping disappear
	 * from the liveness byte while it can still submit direct commands.
	 * In-flight DMA has independent proof references and can finish after
	 * unbind, but CPU mappings/direct calls must be gone first. */
	if (ctx->bar_mapping_count || ctx->direct_count) {
		/* Every profile that can bind, not only the coordinator pair:
		 * a hub or standalone workload can bind, mmap the BAR, unbind,
		 * and keep a VM_PFNMAP mapping alive after a fencer takes the
		 * write lock and reassigns the slot's memory. */
		ret = -EBUSY;
		goto out_unlock;
	}
	...
	lifetime_source = ctx->lifetime_source_file;
	old_tag = ctx->proof_lifetime_tag;
	old_slot = ctx->proof_slot;
	ctx->slot_source_file = NULL;
	ctx->slot_proof_file = NULL;
	ctx->lifetime_proof_file = NULL;
	ctx->lifetime_source_file = NULL;
	...
	/* Publish outside the lock (see mx_lease_transition). Nothing has been
	 * fput yet, so on a fault the same files can go back -- but only while
	 * the ctx is still unbound, i.e. nobody bound anew in between. */
	if (copy_to_user((void __user *)arg, req, sizeof(*req))) {
		ret = -EFAULT;
		mutex_lock(&lease->lock);
		if (!ctx->slot_source_file && !ctx->slot_proof_file &&
		    !ctx->lifetime_proof_file && !ctx->lifetime_source_file) {
			ctx->slot_source_file = slot_source;
			ctx->slot_proof_file = slot_proof;
			ctx->lifetime_proof_file = lifetime_file;
			ctx->lifetime_source_file = lifetime_source;
			ctx->proof_lifetime_tag = old_tag;
			ctx->proof_slot = old_slot;
			slot_source = NULL;
			slot_proof = NULL;
			lifetime_file = NULL;
				      ...
				      unsigned long arg)
{
	...
	if (!mx_lease_valid_header(req.abi_magic, req.abi_version,
	    ...
	    !mx_lease_sm_valid_lifetime_tag(ctx->lease.profile, req.reserved[0]) ||
