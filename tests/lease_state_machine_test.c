...
static void test_hub_replacement(void)
{
	...
	assert(mx_lease_sm_transition(&sm, &publisher,
				      MX_LEASE_TRANSITION_ACTIVATE_WORKLOAD, 0, 0, 0, true) == 0);
	...
	assert(mx_lease_sm_transition(&sm, &replacement,
				      MX_LEASE_TRANSITION_TRY_QUIESCENT, 0, 0, 0, true) == -EBUSY);
	assert(mx_lease_sm_transition(&sm, &replacement,
				      MX_LEASE_TRANSITION_ACTIVATE_WORKLOAD, 0, 0, 0, true) == -EINVAL);
	assert(mx_lease_sm_authorize_direct(&sm, &replacement) == -EACCES);

	mx_lease_sm_release(&sm, &workload);
	assert(mx_lease_sm_transition(&sm, &replacement,
				      MX_LEASE_TRANSITION_TRY_QUIESCENT, 0, 0, 0, true) == 0);
...
static void test_privileged_replacement_transition(void)
{
	...
	assert(replacement.phase == MX_LEASE_PHASE_REPLACEMENT_CANDIDATE);
	assert(mx_lease_sm_transition(&sm, &replacement,
				      MX_LEASE_TRANSITION_ACTIVATE_REPLACEMENT, 0, 0, 0, true) == 0);
...
static void test_idempotence_and_invalid_transitions(void)
{
	...
	assert(mx_lease_sm_transition(&sm, &hub, 99, 0, 0, 0, true) == -EINVAL);
	assert(mx_lease_sm_transition(&sm, &hub,
				      MX_LEASE_TRANSITION_TRY_QUIESCENT, 0, 0, 0, true) == 0);
	assert(mx_lease_sm_transition(&sm, &hub,
				      MX_LEASE_TRANSITION_ACTIVATE_WORKLOAD, 0, 0, 1, true) == -EBUSY);
	assert(mx_lease_sm_transition(&sm, &hub,
				      MX_LEASE_TRANSITION_ACTIVATE_WORKLOAD, 0, 0, 0, true) == 0);
	assert(mx_lease_sm_acquire(&sm, &app,
				   MX_LEASE_PROFILE_HUB_WORKLOAD) == 0);
	assert(mx_lease_sm_transition(&sm, &app,
				      MX_LEASE_TRANSITION_ACTIVATE_WORKLOAD, 0, 0, 0, true) == -EPERM);
...
static void test_slot_domain_anchor_gate(void)
{
	...
	assert(mx_lease_sm_validate_slot_domain_anchor(&publisher,
			MX_LEASE_FAMILY_HUB, true, 0, 0, 0, false, false) == 0);
	assert(mx_lease_sm_validate_slot_domain_anchor(&publisher,
			MX_LEASE_FAMILY_HUB, true, 0, 0, 0, true, true) == 0);
	assert(mx_lease_sm_validate_slot_domain_anchor(&publisher,
			MX_LEASE_FAMILY_HUB, true, 0, 0, 0, true, false) == -EXDEV);
	assert(mx_lease_sm_validate_slot_domain_anchor(&publisher,
			MX_LEASE_FAMILY_HUB, true, 1, 0, 0, false, false) == -EBUSY);
	assert(mx_lease_sm_validate_slot_domain_anchor(&publisher,
			MX_LEASE_FAMILY_HUB, true, 0, 1, 0, false, false) == -EBUSY);
	assert(mx_lease_sm_validate_slot_domain_anchor(&publisher,
			MX_LEASE_FAMILY_HUB, true, 0, 0, 1, false, false) == -EBUSY);
	publisher.phase = MX_LEASE_PHASE_WORKLOAD;
	assert(mx_lease_sm_validate_slot_domain_anchor(&publisher,
			MX_LEASE_FAMILY_HUB, true, 0, 0, 0, false, false) == -EACCES);
