...
_Static_assert(MX_LEASE_CAP_COORDINATED_PARTICIPANTS == (1ULL << 14),
	       "coordinator capability bit");
_Static_assert(MX_LEASE_CAP_WORKLOAD_OFD_LIFETIME == (1ULL << 15),
	       "reopenable workload OFD capability bit");
_Static_assert(MX_LEASE_WORKLOAD_LIVENESS_OFFSET(0) == 8193ULL,
	       "first workload liveness byte avoids XIF cursor and control locks");
_Static_assert(MX_LEASE_WORKLOAD_LIVENESS_OFFSET(MX_LEASE_MAX_WORKLOAD_SLOT) == 16383ULL,
	       "last workload liveness byte");
_Static_assert(MX_LEASE_PROFILE_COORDINATOR_INITIALIZER == 5,
	       "initializer profile is additive");
_Static_assert(MX_LEASE_PROFILE_COORDINATOR_CONTROL == 6,
	       "coordinator controller profile");
_Static_assert(MX_LEASE_PROFILE_COORDINATOR_WORKLOAD == 7,
	       "coordinator workload profile");
_Static_assert(MX_LEASE_SLOT_LIVENESS_OFFSET(0) == 1ULL,
	       "coordinator slot zero liveness byte");
...
_Static_assert(_IOC_TYPE(MX_IOCTL_GET_LEASE_CAPS) == MX_IOCTL_MAGIC,
	       "caps ioctl type");
_Static_assert(_IOC_NR(MX_IOCTL_GET_LEASE_CAPS) == 12,
	       "caps ioctl number");
_Static_assert(_IOC_SIZE(MX_IOCTL_ACQUIRE_LEASE) ==
	       sizeof(struct mx_lease_acquire), "acquire ioctl size");
_Static_assert(_IOC_TYPE(MX_IOCTL_ACQUIRE_LEASE) == MX_IOCTL_MAGIC,
	       "acquire ioctl type");
_Static_assert(_IOC_NR(MX_IOCTL_ACQUIRE_LEASE) == 14,
	       "acquire ioctl number");
_Static_assert(_IOC_SIZE(MX_IOCTL_TRANSITION_LEASE) ==
	       sizeof(struct mx_lease_transition), "transition ioctl size");
_Static_assert(_IOC_TYPE(MX_IOCTL_TRANSITION_LEASE) == MX_IOCTL_MAGIC,
	       "transition ioctl type");
_Static_assert(_IOC_NR(MX_IOCTL_TRANSITION_LEASE) == 15,
	       "transition ioctl number");
_Static_assert(_IOC_SIZE(MX_IOCTL_QUERY_LEASE) ==
	       sizeof(struct mx_lease_query), "query ioctl size");
_Static_assert(_IOC_TYPE(MX_IOCTL_QUERY_LEASE) == MX_IOCTL_MAGIC,
	       "query ioctl type");
_Static_assert(_IOC_NR(MX_IOCTL_QUERY_LEASE) == 13,
	       "query ioctl number");
_Static_assert(_IOC_SIZE(MX_IOCTL_WORKLOAD_PROOFS) ==
	       sizeof(struct mx_lease_proofs), "proofs ioctl size");
_Static_assert(_IOC_TYPE(MX_IOCTL_WORKLOAD_PROOFS) == MX_IOCTL_MAGIC,
	       "proofs ioctl type");
...
_Static_assert(_IOC_TYPE(MX_IOCTL_ANCHOR_SLOT_DOMAIN) == MX_IOCTL_MAGIC,
	       "slot-domain ioctl type");
